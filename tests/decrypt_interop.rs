...
// Decryption of externally-generated fixtures: OpenSSL `cms -encrypt` output
// (via `create-cms encrypt openssl`) and real-world messages (tests/nested/)
...
fn test_decrypt_wrong_key() {
    ...
    let no_recipient = result.failures.iter().any(|f| matches!(f, SmimeError::NoMatchingRecipient));
    assert!(no_recipient, "Expected NoMatchingRecipient, got: {:?}", result.failures);
...
fn test_decrypt_ecdh_with_rsa_key() {
    ...
    let has_key_error = result.failures.iter().any(|f| matches!(f, SmimeError::PrivateKeyParseFailed { .. }));
    assert!(has_key_error, "Expected PrivateKeyParseFailed when using RSA key for ECDH, got: {:?}", result.failures);
}
...
fn test_decrypt_rsa_with_ec_key() {
    ...
    let no_recipient = result.failures.iter().any(|f| matches!(f, SmimeError::NoMatchingRecipient));
    assert!(no_recipient, "Expected NoMatchingRecipient when using EC key for RSA, got: {:?}", result.failures);
}
...
fn test_decrypt_pwri_wrong_password() {
    ...
    let has_pwri_error = result.failures.iter().any(|f| matches!(f, SmimeError::DecryptionFailed { err } if err.starts_with("PWRI")));
    assert!(has_pwri_error, "Expected PWRI unwrap failure, got: {:?}", result.failures);
}
...
fn test_decrypt_pwri_no_password() {
    let eml = fs::read_to_string("tests/general/test_encrypted_pwri.eml").expect("read");
    let result =
        smime::decrypt_and_verify_smime_from_eml_detailed(eml, vec![TrustStore::Debug].into(), &smime::decrypt::DecryptionKeys::default());
    let has_password_error =
        result.failures.iter().any(|f| matches!(f, SmimeError::DecryptionFailed { err } if err.contains("password required")));
    assert!(has_password_error, "Expected missing-password error, got: {:?}", result.failures);
}
