// Signature verification against externally-generated fixtures: OpenSSL `cms -sign`
// (via `create-cms encrypt openssl`), real-world client messages and RFC 4134 vectors.
// Fixtures produced by our own CMS builder belong in cryptography.rs instead.
...
fn test_rfc4134_4_2_signed_rsa_no_attrs() {
    ...
    assert!(result.failures.iter().any(|f| matches!(f, SmimeError::UnsupportedDigestAlg { .. } | SmimeError::DisallowedDigestAlg { .. })));
