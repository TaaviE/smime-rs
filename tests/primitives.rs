...
fn aes_128_cbc_nist_vector() {
    // NIST SP 800-38A F.2.1 CBC-AES128.Decrypt, with a fifth block appended that
    // encrypts the PKCS#7 padding so the vector goes through decrypt_aes_cbc.
    ...
    .unwrap();
    assert_eq!(decrypt_aes_cbc(&key, 16, &iv, &ciphertext).unwrap(), expected_plaintext);
}

#[test]
fn aes_256_cbc_nist_vector() {
    // NIST SP 800-38A F.2.5 CBC-AES256.Decrypt, with a fifth block appended that
    // encrypts the PKCS#7 padding so the vector goes through decrypt_aes_cbc.
